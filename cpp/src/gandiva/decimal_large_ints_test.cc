...
TEST(MultiPrecisionIntTest, Conversion) {
  int128_t x = 0;
  int256_t y = 0;
  EXPECT_TRUE(ConvertToInt256(x) == 0);  // NOLINT

  x = -1;
  EXPECT_TRUE(ConvertToInt256(x) == -1);  // NOLINT
...
TEST(MultiPrecisionFloatTest, Example) {
  const float r_f(static_cast<float>(123) / 100);
  const float a_f = area_of_a_circle(r_f);

  const double r_d(static_cast<double>(123) / 100);
  ...
  ss.str("");
  ss << std::setprecision(numeric_limits<float>::digits10) << a_f;
  EXPECT_EQ(ss.str(), "4.75292");

  ss.str("");
  ss << std::setprecision(std::numeric_limits<double>::digits10) << a_d;
  EXPECT_EQ(ss.str(), "4.752915525616");

  ss.str("");
  ss << std::setprecision(std::numeric_limits<mp::cpp_dec_float_50>::digits10) << a_mp;
