...
TEST(TestArithmeticOps, TestDivide) {
  ...
  int64 out = divide_int64_int64(reinterpret_cast<int64>(&error_holder), 10, true, 0, true,
                                 &is_valid);
  EXPECT_EQ(out, 0);
  ...
  out = divide_int64_int64(reinterpret_cast<int64>(&error_holder), 10, true, 2, true,
                           &is_valid);
  EXPECT_EQ(out, 5);
