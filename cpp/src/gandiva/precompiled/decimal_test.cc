...
void VerifyEquals(const DecimalValue<T>& t1, const DecimalValue<T>& t2) {
  if (t1 != t2) {
    // LOG(ERROR) << t1 << " != " << t2;
    EXPECT_TRUE(false) << t1 << " != " << t2;
  }
}

template <typename T>
void VerifyParse(const std::string& s, int32_t precision, int32_t scale, bool round,
...
template <typename T>
void VerifyParse(const std::string& s, int32_t precision, int32_t scale,
                 const DecimalValue<T>& expected_val,
                 StringParser::ParseResult expected_result) {
...
template <typename T>
void VerifyParse(const std::string& s, int32_t precision, int32_t scale,
                 const DecimalValue<T>& expected_val_v1,
                 StringParser::ParseResult expected_result_v1,
                 const DecimalValue<T>& expected_val_v2,
                 StringParser::ParseResult expected_result_v2) {
...
template <typename T>
void VerifyToString(const T& decimal, int32_t precision, int32_t scale,
                    const std::string& expected) {
  EXPECT_EQ(decimal.ToString(precision, scale), expected);
}

void StringToAllDecimals(const std::string& s, int32_t precision, int32_t scale,
                         int32_t val, StringParser::ParseResult result) {
...
void StringToAllDecimals(const std::string& s, int32_t precision, int32_t scale,
  ...
  VerifyParse(s, precision, scale, Decimal4Value(val_v1), result_v1,
              Decimal4Value(val_v2), result_v2);
  VerifyParse(s, precision, scale, Decimal8Value(val_v1), result_v1,
              Decimal8Value(val_v2), result_v2);
  VerifyParse(s, precision, scale, Decimal16Value(val_v1), result_v1,
              Decimal16Value(val_v2), result_v2);
}
...
TEST(DecimalTest, Overflow) {
  ...
  overflow = false;
  EXPECT_TRUE(result.value() == 0);  // NOLINT
...
TEST(DecimalTest, MultiplyScaleOverflow) {
  ...
  Decimal16Value result =
      x.Multiply<int128_t>(max_scale, y, max_scale, 38, 38, false, &overflow);
  EXPECT_TRUE(result.value() == 0);  // NOLINT
  EXPECT_FALSE(overflow);
  ...
  EXPECT_TRUE(result.value() == 3);  // NOLINT
  EXPECT_FALSE(overflow);
}
...
struct ColumnType {
  static ColumnType CreateDecimalType(int32_t precision, int32_t scale) {
  ...
  switch (op) {
    ...
    case SUBTRACT:
      return ColumnType::CreateDecimalType(
          ...
          max(t1.scale, t2.scale));
    case MULTIPLY:
      return ColumnType::CreateDecimalType(t1.precision + t2.precision,
                                           t1.scale + t2.scale);
    case DIVIDE:
      if (v2) {
...
void VerifyFuzzyEquals(const T& actual, const ColumnType& t, double expected,
                       bool overflow, double max_error = MAX_ERROR) {
  double actual_d = actual.ToDouble(t.scale);
  EXPECT_FALSE(overflow);
  EXPECT_TRUE(fabs(actual_d - expected) < max_error) << actual_d << " != " << expected;
}
...
TEST(DecimalArithmetic, Basic) {
  ...
  VerifyFuzzyEquals(d1.Add<int64_t>(t1.scale, d2, t2.scale, t1_plus_2.precision,
                                    t1_plus_2.scale, false, &overflow),
                    t1_plus_2, d1_double + d2_double, overflow);
  VerifyFuzzyEquals(d1.Add<int64_t>(t1.scale, d3, t2.scale, t1_plus_2.precision,
                                    t1_plus_2.scale, false, &overflow),
                    t1_plus_2, d1_double + d3_double, overflow);
  VerifyFuzzyEquals(d1.Subtract<int64_t>(t1.scale, d2, t2.scale, t1_plus_2.precision,
                                         t1_plus_2.scale, false, &overflow),
                    t1_plus_2, d1_double - d2_double, overflow);
  VerifyFuzzyEquals(d1.Subtract<int64_t>(t1.scale, d3, t2.scale, t1_plus_2.precision,
                                         t1_plus_2.scale, false, &overflow),
                    t1_plus_2, d1_double - d3_double, overflow);
  VerifyFuzzyEquals(d1.Multiply<int128_t>(t1.scale, d2, t2.scale, t1_times_2.precision,
                                          t1_times_2.scale, false, &overflow),
                    t1_times_2, d1_double * d2_double, overflow);
  VerifyFuzzyEquals(d1.Multiply<int64_t>(t1.scale, d3, t2.scale, t1_times_2.precision,
                                         t1_times_2.scale, false, &overflow),
...
TEST(DecimalArithmetic, Divide) {
  ...
  Decimal4Value y(234);
  for (int32_t numerator_p = 1; numerator_p <= 9; ++numerator_p) {
    for (int32_t numerator_s = 0; numerator_s <= numerator_p; ++numerator_s) {
      for (int32_t denominator_p = 1; denominator_p <= 3; ++denominator_p) {
        for (int32_t denominator_s = 0; denominator_s <= denominator_p; ++denominator_s) {
          for (int32_t v2 : {0, 1}) {
            ...
            Decimal8Value r = x.Divide<int64_t>(t1.scale, y, t2.scale, t3.precision,
                                                t3.scale, true, &is_nan, &is_overflow);
            ...
            if (fabs(approx_r - expected_r) > MAX_ERROR) {
              EXPECT_TRUE(false)
                  << approx_r << " " << expected_r << x.ToString(t1.precision, t1.scale)
                  << "/" << y.ToString(t2.precision, t2.scale) << "="
                  << r.ToString(t3.precision, t3.scale);
            }
          }
        }
      }
  ...
  bool is_overflow = false;
  Decimal8Value r =
      x.Divide<int64_t>(0, Decimal4Value(0), 0, 38, 4, true, &is_nan, &is_overflow);
...
TEST(DecimalArithmetic, DivideLargeScales) {
  ...
  Decimal16Value x = StringParser::StringToDecimal<int128_t>(
      data, strlen(data), t1.precision, t1.scale, false, &result);
  ...
  y = -y;
  r = x.Divide<int128_t>(t1.scale, y, t2.scale, t3.precision, t3.scale, true, &is_nan,
                         &is_overflow);
  VerifyToString(r, t3.precision, t3.scale, "-31939128.06356147605500000000000000000");
  EXPECT_FALSE(is_nan);
  EXPECT_FALSE(is_overflow);
...
template <typename T>
DecimalValue<T> RandDecimal(int32_t max_precision) {
  T val = 0;
  int32_t precision = rand_r(&seed) % max_precision;
  for (int32_t i = 0; i < precision; ++i) {
    int32_t digit = rand_r(&seed) % 10;
    val = val * 10 + digit;
  }
  return DecimalValue<T>(rand_r(&seed) % 2 == 0 ? val : -val);
}

int32_t DoubleCompare(double x, double y) {
  if (x < y) return -1;
  if (x > y) return 1;
  return 0;
}
...
TEST(DecimalArithmetic, RandTesting) {
  int32_t NUM_ITERS = 1000000;
  for (int32_t i = 0; i < NUM_ITERS; ++i) {
    // TODO: double is too imprecise so we can't test with high scales.
    int32_t p1 = rand_r(&seed) % 12 + 1;
    int32_t s1 = rand_r(&seed) % min(4, p1);
    int32_t p2 = rand_r(&seed) % 12 + 1;
    int32_t s2 = rand_r(&seed) % min(4, p2);
    ...
    bool overflow = false;
    VerifyFuzzyEquals(dec1.Add<int64_t>(t1.scale, dec2, t2.scale, add_t.precision,
                                        add_t.scale, false, &overflow),
                      add_t, t1_d + t2_d, overflow);
    VerifyFuzzyEquals(dec1.Subtract<int64_t>(t1.scale, dec2, t2.scale, add_t.precision,
                                             add_t.scale, false, &overflow),
                      add_t, t1_d - t2_d, overflow);
    ...
    EXPECT_EQ(dec1.Compare(t1.scale, dec1, t1.scale), 0);
    EXPECT_EQ(dec2.Compare(t2.scale, dec2, t2.scale), 0);
  }
}
