...
template <typename T>
inline DecimalValue<T> DecimalValue<T>::FromDouble(int32_t precision, int32_t scale,
                                                   double d, bool round, bool* overflow) {
  ...
  DCHECK(max_value > 0);  // NOLINT no DCHECK_GT because of int128_t
  if (ARROW_PREDICT_FALSE(std::isnan(d)) ||
      ARROW_PREDICT_FALSE(std::fabs(d) >= max_value)) {
    *overflow = true;
    return DecimalValue();
  }
...
template <typename T>
inline DecimalValue<T> DecimalValue<T>::FromInt(int32_t precision, int32_t scale,
                                                int64_t d, bool* overflow) {
...
template <typename T>
inline int32_t DecimalValue<T>::Compare(const DecimalValue& other) const {
  T x = value();
  T y = other.value();
  if (x == y) return 0;
  if (x < y) return -1;
...
template <typename T>
inline const T DecimalValue<T>::whole_part(int32_t scale) const {
  return value() / DecimalUtil::GetScaleMultiplier<T>(scale);
}

template <typename T>
inline const T DecimalValue<T>::fractional_part(int32_t scale) const {
...
inline typename RESULT_T::underlying_type_t DecimalValue<T>::ToInt(int32_t scale,
                                                                   bool* overflow) const {
  ...
  if (divisor == 1) {
    ...
    DCHECK(divisor % 2 == 0);  // NOLINT No DCHECK_EQ as this is possibly an int128_t
...
template <typename T>
inline DecimalValue<T> DecimalValue<T>::ScaleTo(int32_t src_scale, int32_t dst_scale,
                                                int32_t dst_precision,
                                                bool* overflow) const {
  int32_t delta_scale = src_scale - dst_scale;
  T result = value();
...
inline int32_t MaxBitsRequiredAfterScaling(int128_t num, int32_t scale_by) {
  ...
  int32_t num_occupied = 128 - BitUtil::CountLeadingZeros<int128_t>(abs(num));
  DCHECK_GE(scale_by, 0);
  DCHECK_LE(scale_by, 76);
...
template <typename T>
inline int32_t MinLeadingZeros(T x, int32_t x_scale, T y, int32_t y_scale) {
  int32_t x_lz = BitUtil::CountLeadingZeros<T>(abs(x));
  int32_t y_lz = BitUtil::CountLeadingZeros<T>(abs(y));
  if (x_scale < y_scale) {
...
inline void SeparateFractional(int128_t x, int32_t x_scale, int128_t y, int32_t y_scale,
                               int128_t* x_left, int128_t* x_right, int128_t* y_left,
                               int128_t* y_right) {
  // The whole part.
...
inline int128_t AddLarge(int128_t x, int32_t x_scale, int128_t y, int32_t y_scale,
                         int32_t result_scale, bool round, bool* overflow) {
  DCHECK(x >= 0 && y >= 0);
  ...
  int32_t max_scale = std::max(x_scale, y_scale);
  int32_t result_scale_decrease = max_scale - result_scale;
  DCHECK_GE(result_scale_decrease, 0);

  // carry_to_left should be 1 if there is an overflow when adding the fractional parts.
  int32_t carry_to_left = 0;
  if (ARROW_PREDICT_FALSE(
          x_right >= DecimalUtil::GetScaleMultiplier<int128_t>(max_scale) - y_right)) {
  ...
  if (result_scale_decrease > 0) {
    right = DecimalUtil::ScaleDownAndRound<int128_t>(right, result_scale_decrease, round);
  }
  DCHECK(right >= 0);  // NOLINT
...
inline int128_t SubtractLarge(int128_t x, int32_t x_scale, int128_t y, int32_t y_scale,
                              int32_t result_scale, bool round, bool* overflow) {
  DCHECK(x != 0 && y != 0);
  DCHECK((x > 0) != (y > 0));
...
template <typename T>
template <typename RESULT_T>
inline DecimalValue<RESULT_T> DecimalValue<T>::Add(
    int32_t this_scale, const DecimalValue& other, int32_t other_scale,
    int32_t result_precision, int32_t result_scale, bool round, bool* overflow) const {
  ...
  int32_t min_lz =
      detail::MinLeadingZeros(abs(value()), this_scale, abs(other.value()), other_scale);
  int32_t result_scale_decrease =
      std::max(this_scale - result_scale, other_scale - result_scale);
  DCHECK_GE(result_scale_decrease, 0);

  const int32_t MIN_LZ = 3;
  if (min_lz >= MIN_LZ) {
  ...
  if (x >= 0 && y >= 0) {
    result =
        detail::AddLarge(x, this_scale, y, other_scale, result_scale, round, overflow);
  } else if (x <= 0 && y <= 0) {
    result =
        -detail::AddLarge(-x, this_scale, -y, other_scale, result_scale, round, overflow);
  } else {
    result = detail::SubtractLarge(x, this_scale, y, other_scale, result_scale, round,
                                   overflow);
  }
...
template <typename T>
template <typename RESULT_T>
DecimalValue<RESULT_T> DecimalValue<T>::Multiply(
    int32_t this_scale, const DecimalValue& other, int32_t other_scale,
    int32_t result_precision, int32_t result_scale, bool round, bool* overflow) const {
...
template <typename T>
template <typename RESULT_T>
inline DecimalValue<RESULT_T> DecimalValue<T>::Divide(
    int32_t this_scale, const DecimalValue& other, int32_t other_scale,
    int32_t result_precision, int32_t result_scale, bool round, bool* is_nan,
    bool* overflow) const {
  DCHECK_GE(result_scale + other_scale, this_scale);
  if (ARROW_PREDICT_FALSE(other.value() == 0)) {
    // Divide by 0.
    *is_nan = true;
  ...
  int32_t scale_by = result_scale + other_scale - this_scale;
  DCHECK_GE(scale_by, 0);
  // Use higher precision ints for intermediates to avoid overflows. Divides lead to
  // large numbers very quickly (and get eliminated by the int32_t divide).
  if (sizeof(T) == 16) {
    int128_t x_sp = value();
    ...
    int256_t x =
        DecimalUtil::MultiplyByScale<int256_t>(ConvertToInt256(x_sp), scale_by, ovf);
    *overflow |= ovf;
    ...
    if (round) {
      ...
      if (abs(2 * remainder) >= abs(y)) {
        ...
        DCHECK(r != 0);  // NOLINT DCHECK_NE doesn't work with int128_t
        r += BitUtil::Sign(r);
      }
    }
...
template <typename T>
template <typename RESULT_T>
inline DecimalValue<RESULT_T> DecimalValue<T>::Mod(int32_t this_scale,
                                                   const DecimalValue& other,
                                                   int32_t other_scale,
                                                   int32_t result_precision,
                                                   int32_t result_scale, bool round,
                                                   bool* is_nan, bool* overflow) const {
  DCHECK_EQ(result_scale, std::max(this_scale, other_scale));
  *is_nan = other.value() == 0;
  if (ARROW_PREDICT_FALSE(*is_nan)) return DecimalValue<RESULT_T>();
  ...
  switch (sizeof(RESULT_T)) {
    ...
    case 8: {
      if (detail::MinLeadingZeros<RESULT_T>(value(), this_scale, other.value(),
                                            other_scale) >= 2) {
        int64_t x, y;
...
template <typename T>
template <typename RESULT_T>
inline void DecimalValue<T>::AdjustToSameScale(const DecimalValue<T>& x, int32_t x_scale,
                                               const DecimalValue<T>& y, int32_t y_scale,
                                               int32_t result_precision,
                                               RESULT_T* x_scaled, RESULT_T* y_scaled) {
  int32_t delta_scale = x_scale - y_scale;
...
template <>
inline int32_t Decimal4Value::Compare(int32_t this_scale, const Decimal4Value& other,
                                      int32_t other_scale) const {
  int64_t x, y;
...
template <>
inline int32_t Decimal8Value::Compare(int32_t this_scale, const Decimal8Value& other,
                                      int32_t other_scale) const {
  int128_t x = 0, y = 0;
  AdjustToSameScale(*this, this_scale, other, other_scale, 0, &x, &y);
  if (x == y) return 0;
...
template <>
inline int32_t Decimal16Value::Compare(int32_t this_scale, const Decimal16Value& other,
                                       int32_t other_scale) const {
  int256_t x = ConvertToInt256(this->value());
  int256_t y = ConvertToInt256(other.value());
  int32_t delta_scale = this_scale - other_scale;
  if (delta_scale > 0) {
...
template <typename T>
inline std::string DecimalValue<T>::ToString(int32_t precision, int32_t scale) const {
  ...
  int32_t last_char_idx = precision + (scale > 0)  // Add a space for decimal place
  ...
  // point32_t the last_char_idx points to the string terminator.
  T remaining_value = value_;
  int32_t first_digit_idx = 0;
  if (value_ < 0) {
    remaining_value = -value_;
    first_digit_idx = 1;
  }
  if (scale > 0) {
    int32_t remaining_scale = scale;
    do {
...
template <typename T>
inline double DecimalValue<T>::ToDouble(int32_t scale) const {
...
inline Decimal4Value ToDecimal4(const Decimal4Value& v, bool* overflow) { return v; }
...
inline Decimal8Value ToDecimal8(const Decimal8Value& v, bool* overflow) { return v; }
...
inline Decimal16Value ToDecimal16(const Decimal16Value& v, bool* overflow) { return v; }
